...
class Arlima_Plugin {
    ...
    function displayArlimaList() {
        ...
        if( $list_id ) {
            ...
            $list = Arlima_ListFactory::load($list_id, $version);
            if( $list->exists ) {
    ...
    public function arlimaListShortCode($atts) {
        ...
        $list = Arlima_ListFactory::load($atts['list']);

        if( !$list->exists ) {
            return sprintf($error_html, 'Short code [arlima] is referring to a list that does not exist ('.$atts['list'].')');
        }
    ...
    public function adminBar() {
        ...
        if( is_admin_bar_showing() ) {
            ...
            $wp_admin_bar->add_menu( array(
                    ...
                    'href' => admin_url( 'admin.php?page=arlima' )
                ) );
            foreach(Arlima_ListFactory::loadListSlugs() as $list_data) {
                $wp_admin_bar->add_menu( array(
                        'id' => 'arlima-'.$list_data->id,
                        'parent' => 'arlima',
                        'title' => $list_data->title,
                        'href' => admin_url( 'admin.php?page=arlima&amp;open_list='.$list_data->id )
                    ) );
            }
        }
    }
            ...
            if( $current_version < self::VERSION ) {
                ...
                foreach($pages as $page) {
                    ...
                    if( $arlima_slug ) {
                        ...
                        update_post_meta($page->ID, '_arlima_list_data', array(
    ...
    function postMetaBox() {
        ...
        if ( $post->post_status == 'publish' ) { ?>
            ...
            <select name="arlima-listid" id="arlima-listid">
                ...
                <?php foreach( Arlima_ListFactory::loadListSlugs() as $arlima_list ) { ?>
                    <option value="<?php echo $arlima_list->id; ?>"><?php echo $arlima_list->title; ?></option>
                <?php }	?>
            </select>
            <input id="arlima-send-to-list-btn" class="button-secondary action" type="button" name="arlima-send-to-list-btn" value="Skicka" /> <img src="<?php echo   ARLIMA_PLUGIN_URL . '/images/ajax-loader-trans.gif'; ?>" class="ajax-loader" style="display:none;" />
        <?php }else{
        ...
        <div id="arlima-list-settings">
            <table cellpadding="5">
                        ...
                        <select name="arlima_list">
                            ...
                            <?php foreach( Arlima_ListFactory::loadListSlugs() as $arlima_list ): ?>
                                <option value="<?php echo $arlima_list->id; ?>"<?php
                                        if( in_array($list_data['list'], (array)$arlima_list) )  // may be either slug or id
                ...
                <tr class="arlima-opt">
                    ...
                    <td><input type="number" name="arlima_width" value="<?php echo $list_data['width'] ?>" style="width:50px" /> px</td>
                </tr>
                <tr>
        ...
        if( !defined('DOING_AUTOSAVE') || !DOING_AUTOSAVE ) {
            ...
            if( isset($_POST['arlima_nonce']) && wp_verify_nonce($_POST['arlima_nonce'], __FILE__) ) {
                if( empty($_POST['arlima_list']) ) {
                    ...
                    update_post_meta($post_id, '_arlima_list_data', array(
     ...
     */
    function adminMenu() {
        $arlima_mainpage = add_menu_page(__('Article List Manager', 'arlima'), __('Article lists', 'arlima'), 'edit_posts', 'arlima', 'Arlima_Plugin::loadAdminMainPage', ARLIMA_PLUGIN_URL . '/images/skull-icon.png');
        add_submenu_page('arlima', __('Article lists', 'arlima'), __('Manage lists', 'arlima'), 'edit_posts', 'arlima', 'Arlima_Plugin::loadAdminMainPage');
        $arlima_editpage = add_submenu_page('arlima', __('Edit lists', 'arlima'), __('Edit lists', 'arlima'), 'edit_posts', 'arlima-editpage', 'Arlima_Plugin::loadAdminEditPage' );
        $arlima_webservicepage = add_submenu_page('arlima',  __('Web Service', 'arlima'), __('Web Service', 'arlima'), 'edit_posts', 'arlima-webservicepage', 'Arlima_Plugin::loadAdminWebServicePage');
    ...
    function addAdminPageScriptsAndStylesheets($main_page_slug, $edit_page_slug, $service_page_slug) {
        ...
        if($php_file == 'post-new.php' || $php_file = 'post.php') {
            wp_enqueue_script('arlima_js_admin', ARLIMA_PLUGIN_URL.'/js/admin-post.js', array('jquery' ), self::STATIC_VERSION );
            wp_localize_script('arlima_js_admin', 'ArlimaJSAdmin', array(
                    'ajaxurl' => admin_url('admin-ajax.php' ),
                    'arlimaNonce' => wp_create_nonce('arlima-nonce' ),
                    'imageurl' => ARLIMA_PLUGIN_URL . '/images/',
                    'lang' => array(
                        'notice' => __('Are you sure you want to insert this post in the top of selected article list?', 'arlima'),
    ...
    function addAdminEditPageScripts() {
        ...
        wp_enqueue_script( 'arlima_js_jquery', ARLIMA_PLUGIN_URL . 'js/arlima/arlima-jquery-plugins.js', array( 'jquery' ), self::STATIC_VERSION );
    }
    ...
    function addAdminMainPageScripts() {
        ...
        wp_register_script(  'jquery-ui', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.16/jquery-ui.js', array( 'jquery' ) );
        ...
        wp_enqueue_script( 'colourpicker', ARLIMA_PLUGIN_URL . 'js/jquery/colourpicker/jquery.colourpicker.js', array( 'jquery' ) );

        wp_register_script( 'fancybox',  ARLIMA_PLUGIN_URL . 'js/jquery/fancybox/jquery.fancybox-1.3.4.pack.js', 'jquery', null, true);
        wp_enqueue_script( 'fancybox' );

        wp_enqueue_script( 'jquery-ui-nestedsortable', ARLIMA_PLUGIN_URL . 'js/jquery/jquery.ui.nestedSortable.js', array( 'jquery' ), '1.2' );
        ...
        wp_enqueue_script( 'arlima-jquery-plugins', ARLIMA_PLUGIN_URL . 'js/arlima/arlima-jquery-plugins.js', array( 'jquery' ),self::STATIC_VERSION );
        wp_enqueue_script( 'arlima-template-loader', ARLIMA_PLUGIN_URL . 'js/arlima/template-loader.js', array( 'jquery' ), self::STATIC_VERSION );
        wp_enqueue_script( 'arlima-js', ARLIMA_PLUGIN_URL . 'js/arlima/arlima.js', array( 'jquery' ),  self::STATIC_VERSION );
        wp_enqueue_script( 'arlima-plupload-init', ARLIMA_PLUGIN_URL . 'js/arlima/plupload-init.js', array( 'jquery' ), self::STATIC_VERSION );
        wp_enqueue_script( 'arlima-main-js', ARLIMA_PLUGIN_URL . 'js/page-main.js', array( 'jquery' ), self::STATIC_VERSION );

        // Use the newest version of hotkeys (wordpress uses old beta)
        wp_deregister_script('jquery-hotkeys');
        wp_register_script('jquery-hotkeys-new-version', ARLIMA_PLUGIN_URL . 'js/jquery/jquery.hotkeys.js', array(), self::STATIC_VERSION );
        wp_enqueue_script('jquery-hotkeys-new-version');

        wp_localize_script( 'arlima-js', 'ArlimaJS', array(
                'ajaxurl' => admin_url( 'admin-ajax.php' ),
                'arlimaNonce' => wp_create_nonce( 'arlima-nonce' ),
                'imageurl' => ARLIMA_PLUGIN_URL . '/images/',
    ...
    function addAdminStyleSheets() {
        ...
        wp_enqueue_style('jquery_ui_css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.8/themes/smoothness/jquery-ui.css' );
    ...
    public static function classLoader($class) {
        ...
        elseif( substr($class, 0, 10) == 'jQueryTmpl' ) {
            $jquery_tmpl_class = ARLIMA_PLUGIN_PATH.'/classes/jquery-tmpl-php/'.str_replace('_', DIRECTORY_SEPARATOR, $class).'.php';
    ...
    public static function warnAboutUseOfDeprecatedFunction($func, $since, $replacement=false) {
        if(WP_DEBUG) {
            trigger_error( sprintf('Use of deprecated arlima function %s, deprecated since vesion %f %s', $func, (string)$since, $replacement ? ' use '.$replacement.' instead':''), E_USER_WARNING );
        }
    }
    ...
    protected static function loadAdminPage($page) {
        ?>
        <div class="wrap"><div id="icon-plugins" class="icon32"></div>
